...
- Every unsafe request (`POST`, `PUT`, `PATCH`, and `DELETE`) must include the signed CSRF value in both the CSRF cookie and `X-CSRF-Token` header. Signatures are checked using constant-time comparison.
...
- Redis-backed fixed-window limits protect login and password-reset routes. The limiter fails closed if Redis is unavailable.
- IP addresses are represented in audit data and rate-limit fingerprints by keyed hashes rather than plaintext.

Development cookie names are `webhost_billing_session` and `webhost_billing_csrf`. Production cookie names are `__Host-webhost_billing_session` and `__Host-webhost_billing_csrf`.
...
| `POST`   | `/auth/login`                        | Public + CSRF + rate limit | Start a session                      |
...
Security-sensitive actions append activity records for registration, verification, successful and failed login, reset request/completion, logout, logout-all, and individual session revocation. Failed-login metadata does not contain a password or session token.
...
- `CREDENTIAL_ENCRYPTION_KEY` (at least 32 characters and distinct in real environments)
- `SESSION_TTL_SECONDS`
...
The authentication suite uses isolated fictional `.test` accounts and cleans only those test identities. It covers successful registration, verification and login; generic invalid credentials; expired and reused tokens; role denial; cross-customer access denial; session revocation after password reset; logout-all; and administrator authorization.
